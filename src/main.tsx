...
import { maxChroma, oklabToRgb, rgbCss, scoreGuess, wheelToRgb, type RGB } from '../shared/color.ts';
...
function App() {
  ...
  const [picker, setPicker] = useState({ L: 0.68, hue: 0.25, radius: 0.7 });
  ...
  useEffect(() => { setClue(''); setPicker({ L: 0.68, hue: 0.25, radius: 0.7 }); }, [view?.roundNumber]);
  ...
  return <div className="site-shell">
    ...
    {!view ? <main className="welcome layout">
      ...
      <section className="game-main">
        ...
        {view.phase === 'guess' && <div className="panel round-panel"><div className="round-kicker">ПОДСКАЗКА ОТ {describer?.nickname?.toUpperCase()} <span className="countdown">◷ {secondsLeft} с</span></div><blockquote>{view.clue}</blockquote>{self?.isDescriber ? <><div className="secret-color compact" style={{ background: view.target ? rgbCss(view.target) : undefined }} /><p>Вы уже описали цвет. Ждём ответы остальных игроков.</p></> : self?.hasGuessed ? <div className="submitted"><span>✓</span><h2>Ответ принят</h2><p>Результаты появятся после ответов всех игроков или окончания времени.</p></div> : <><p className="hint">Найдите оттенок на круге и установите светлоту. Ответ можно отправить один раз.</p><ColorWheel value={picker} onChange={setPicker} /><button className="button primary guess-button" disabled={busy || secondsLeft <= 0} onClick={() => action('guess', selectedRgb)}>Отправить этот цвет →</button></>}</div>}
        {view.phase === 'reveal' && <div className="panel round-panel"><div className="round-kicker">ИТОГИ РАУНДА {view.roundNumber}</div>{view.skipped ? <><h2>Раунд пропущен</h2><p>Ведущий отключился до публикации подсказки. Очки не начислены.</p></> : <><h2>Вот какой цвет был загадан</h2><div className="answer-hero"><span className="answer-swatch" style={{ background: view.target ? rgbCss(view.target) : undefined }} /><div><span>Подсказка</span><strong>«{view.clue}»</strong><small>Ведущий {describer?.nickname}: +{view.describerPoints} очков</small></div></div><div className="results-list">{view.results?.map((result) => <div className="result-row" key={result.playerId}><span className="result-avatar">{result.avatar}</span><span className="result-name">{result.nickname}</span><span className="result-color" style={{ background: result.color ? rgbCss(result.color) : 'transparent' }} /> <span className="result-distance">{result.distance === null ? 'Нет ответа' : `Δ ${result.distance.toFixed(3)}`}</span><strong>+{result.points}</strong></div>)}</div></>}{isHost ? <button className="button primary next-button" disabled={busy} onClick={() => action('next')}>{view.roundNumber === view.totalRounds ? 'Показать победителя →' : 'Следующий раунд →'}</button> : <div className="notice">Следующий раунд запустит хозяин комнаты.</div>}</div>}
        {view.phase === 'finished' && <div className="panel finish-panel"><div className="finish-symbol">✦</div><div className="eyebrow">ПОБЕДИТЕЛЬ ПАРТИИ</div><h2>{view.history[0]?.winners.join(' и ') || 'Игра окончена'}</h2><p>Все цвета разгаданы. Можно начать новую партию с этим же кодом комнаты.</p><div className="final-list">{view.history[0]?.leaderboard.map((player, index) => <div key={`${player.nickname}-${index}`}><span>{index + 1}</span><span>{player.avatar} {player.nickname}</span><strong>{player.score.toLocaleString('ru-RU')}</strong></div>)}</div>{isHost && <button className="button primary" disabled={busy} onClick={() => action('new_game')}>Новая партия →</button>}</div>}
